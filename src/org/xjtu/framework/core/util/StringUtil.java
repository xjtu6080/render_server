...
public class StringUtil {
	...
	/**
	 * 功能描述：判断输入的字符串是否符合手机号码样式.
	 * 
	 * @param str
	 *            传入的字符串
	 * @return 是手机号码样式返回true,否则返回false
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println("字符串：" + isLetterOrNumber(""));
		System.out.println("字符串：" + isLetterOrNumber("123"));
		System.out.println("字符串：" + isLetterOrNumber("dsa@123"));
		System.out.println("字符串：" + isLetterOrNumber("dsa_123"));
		System.out.println("字符串：" + isLetterOrNumber(".dsa123"));
	...
	/**
	 * 返回一个拼接成逗号间隔的连续数字字符串
	 * @param frameRange，输入类型为3,4,5,7-9混合而成的字符串
	 * @param 
	 * @return
	 */
	public static String FormateFrameNumberStr(String frameRange){
		String res="";
	...
	//给出一个祯范围，采用统计学均匀分布的方法方法抽出固定的祯数，返回选中的祯数的编号
	//这里范围统一为丢好间隔的排好序的方式
	public static  String[] getSelectFrame(String frameRange,int FrameNumber){
		frameRange=StringUtil.FormateFrameNumberStr(frameRange);
		String result[]=new String[FrameNumber];
		int j=0;
		System.out.println(frameRange);
		String[]franmeNo=frameRange.split(",");
		int step=franmeNo.length/FrameNumber;
		for (int i = 0; i < franmeNo.length; i+=step) {
			
			if(j<=FrameNumber-1)
			result[j++]=franmeNo[i];
		}
		System.out.println("select frame is"+Arrays.toString(result));
	return result;	

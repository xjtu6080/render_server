...
public class SSLConnetionUtils {
	...
	// Java Trust Store的路径
	private static final String JAVA_HOME = "C:\\hadoop\\Java\\jdk1.7.0\\jre";
	private static final String JAVA_HOME_SYS = System.getProperty("java.home");//获得java系统变量
	...
	public static void importCertificate() {
		try {
			...
			KeyStore ks = KeyStore.getInstance("JKS");
			ks.load(null, trustStorePassword.toCharArray());//从指定输入流载入keystore
			setRemainCertificates(ks, jksPath, trustStorePassword);//保留原有证书
			Certificate certificate = getUpopCertificate();
			ks.setCertificateEntry(certFileAlias, certificate);//给指定可信证书起一个别名
			ks.store(new FileOutputStream(jksPath), trustStorePassword.toCharArray());//把keystore存入jkspath
			System.out.println("Import Certificate " + certFileAlias + " Successful!");
		} catch (Exception e) {
			throw new RuntimeException("Exception while importing certificate to Java Key Store for unionpaysecure certificate.",
					e);
	...
	public static void main(String[] args) throws Exception {
		...
		///deleteCertificate(certFileAlias);//删除原有证书
		//importCertificate();//导入证书
		//printCertificateInfo();//打印证书信息
	}
}
